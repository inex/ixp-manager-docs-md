...
As an example, as the time of writing, 7.4.x is the latest release and 7.5 is in development. We did **our final** push to 7.4 via:
...
Once 7.5 is released, we will push a final update to 7.5 updating it to latest:

```sh
PATH=./venv/bin:$PATH ./venv/bin/mike deploy --push --update-aliases 7.5 latest
```

And all new documentation will be pushed to dev via:

```sh
PATH=./venv/bin:$PATH ./venv/bin/mike deploy --push --update-aliases 7.6 dev
```
